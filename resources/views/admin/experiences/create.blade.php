<x-admin.layouts.app :title="'Create'">
    <div class="w-full rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <form method="POST" action="{{ route('admin.experiences.store') }}">
            ...
            <div class="space-y-4">
                <x-admin.form-input name="role" label="Role / Designation" required placeholder="E.g. Article Assistant" />
                <x-admin.form-input name="firm_name" label="Firm Name" placeholder="E.g. ABC & Associates" />
                <x-admin.form-input name="location" label="Location" placeholder="E.g. Ahmedabad, Gujarat" />
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <x-admin.form-input name="start_date" label="Start Date" type="date" required placeholder="YYYY-MM-DD" />
                    <x-admin.form-input name="end_date" label="End Date" type="date" placeholder="YYYY-MM-DD" help="Leave blank if currently working" />
                </div>
                <x-admin.form-checkbox name="is_current" label="Currently working here" :checked="false" />
                <x-admin.form-textarea name="description" label="Description" rows="4" placeholder="Describe your role and key achievements..." />
            ...
            <div class="mt-6 flex items-center gap-3 border-t border-slate-100 pt-5">
                <button type="submit" class="rounded-lg bg-navy-900 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-navy-800">Create Experience</button>
