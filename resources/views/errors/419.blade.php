<x-frontend.layouts.app :seoTitle="'419 | Session Expired'">
    <section class="relative overflow-hidden bg-night">
        ...
        <div class="container-app relative flex min-h-screen flex-col items-center justify-center py-32 text-center">
            <p class="font-display text-8xl font-bold md:text-9xl">
                <span class="bg-gradient-to-r from-violet-300 via-violet-400 to-violet-200 bg-clip-text text-transparent">419</span>
            </p>
            <h1 class="mt-4 text-2xl font-semibold text-white md:text-3xl">Session Expired</h1>
            <p class="mx-auto mt-3 max-w-md text-sm text-slate-400">
                Your session has expired due to inactivity. Please go back and try again.
            </p>
            <div class="mt-8 flex flex-wrap justify-center gap-3">
                <a href="{{ url()->previous() }}" class="rounded-lg bg-gradient-to-r from-violet-500 to-violet-700 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-700/30 transition-all hover:-translate-y-0.5 hover:from-violet-400 hover:to-violet-600">Try Again</a>
                <a href="{{ url('/') }}" class="rounded-lg border border-white/20 bg-night/5 px-6 py-3 text-sm font-semibold text-white backdrop-blur transition-all hover:bg-night/10">Back to Home</a>
            </div>
        </div>
    </section>
